...
<div class="container">
    ...
    {#if activeButton === 'preview' || activeButton === ''}
        <div class="preview" bind:this={previewRef} style="width: {previewWidth};">
            {@html postInfo.postContent ? marked(postInfo.postContent.replace(/!\[(.*?)\]\((.*?)\)/g, '<div class="image"style="text-align: center; margin: 1em 0 1em 0"><img src="$2" alt="$1" style="max-width: 100%; height: auto;"/></div>')) : 'Post content not found.'}
        </div>
    {/if}
</div>
